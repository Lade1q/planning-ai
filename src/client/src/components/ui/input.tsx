...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'border-border rounded-field bg-card duration-(--duration-fast) ease-(--ease-standard) h-9.5 w-full min-w-0 border px-3 py-2.25 text-base outline-none transition-colors',
