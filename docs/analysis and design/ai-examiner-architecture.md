...
the AI is called only at four fixed points and never decides *what happens next*.
...
orchestrates.** Everything that decides *what happens next* is deterministic application code and
