...
const clientEslint = path.join(__dirname, 'src', 'client', 'node_modules', 'eslint', 'bin', 'eslint.js')
const serverEslint = path.join(__dirname, 'src', 'server', 'node_modules', 'eslint', 'bin', 'eslint.js')
...
module.exports = {
  ...
  'src/client/**/*.{ts,tsx}': (files) => {
    ...
    return [
      `node "${clientEslint}" --fix ${fileList}`,
      `node "${prettier}" --write ${fileList}`,
    ]
  },
  ...
  'src/server/**/*.ts': (files) => {
    const fileList = files.map((f) => `"${f}"`).join(' ')
    return [
      `node "${serverEslint}" --fix ${fileList}`,
      `node "${prettier}" --write ${fileList}`,
    ]
  },
